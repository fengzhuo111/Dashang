    ...
    export default {
        ...
        data() {
            var validateIDNumber = (rule, value, callback) => {
                var reg = /^\d{17}[X\d]$/;  // 17位数字+最后一位为数字或X
                if (value === '') {
                    callback(new Error('请输入身份证号'));
                } else if (!reg.test(this.modifyForm.IDNumber)) {
            ...
            var validateEmail = (rule, value, callback) => {
                ...
                } else if (!reg.test(this.modifyForm.email)) {
            ...
            var validateTelephone = (rule, value, callback) => {
                var reg = /^[1]\d{10}$/;  //12位数字，1开头
                if (value === '') {
                    callback(new Error('请输入电话号码'));
                } else if (!reg.test(this.modifyForm.telephone)) {
